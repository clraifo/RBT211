/*
Two external LEDs alternate blinking in the default state. When the ISR is triggered by the pushbutton, they are turned off and the onboard LED 
blinks at the same rate. The blink rate of the LEDs can be adjusted by changing the value of the Output Compare Register A (OCR1A) for Timer1.  

To increase the blink rate (make the LEDs blink more frequently), decrease the value of OCR1A. To decrease the blink rate (make the LEDs blink 
less frequently), increase the value of OCR1A. The actual blink rate can be calculated using the following formula:
...
This version maintains a global button_flag variable to keep track of whether the button is currently pressed. This flag determines which 
LED(s) to toggle in the Timer1 compare match ISR.
...
ISR(INT0_vect){
	if (button_flag == 0){
		PORTD &= ~((1 << LED0) | (1 << LED1));  // Turns OFF both LEDs
		button_flag = 1;
		} else {
		PORTD &= ~((1 << LED0) | (1 << LED1));  // Turns OFF both LEDs
		PORTB &= ~(1 << ONBOARD_LED);           // Turns OFF onboard LED
		// resume the alternate blinking pattern
		PORTD |= (1 << LED0);                   // Turns ON LED0
		PORTD &= ~(1 << LED1);                  // Turns OFF LED1
		button_flag = 0;
	}
}
